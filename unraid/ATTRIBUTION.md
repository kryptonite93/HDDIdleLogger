...
If setup fails before `proof_running`, no capture has started. Diagnostic version 2 prints `proof_error` with the exact failed operation, tracefs path, requested setting or probe definition, error number and any matching kernel diagnostics before cleanup. Return that line together with the cleanup result. Shared kernel error logs are read without clearing them; unrelated probe errors are excluded. The first host attempt successfully inspected libfuse but failed with `EINVAL` during setup; the original output did not identify the rejected operation, so runtime compatibility remains unverified.
...
Output contains process/thread IDs, process names, container IDs and disk numbers, with at most 20 examples. File paths are read temporarily by the trace instance to identify the backing mount but are not printed or written into the profiler database. Trace-buffer loss aborts the run and invalidates all candidates from that run.
