...
const columns = [
  ...
  ["longest_idle_seconds","Longest idle",d=>duration(d.longest_idle_seconds),d=>d.longest_idle_seconds],
  ["median_idle_seconds","Median idle",d=>duration(d.median_idle_seconds),d=>d.median_idle_seconds],
...
async function renderEvents() {
  ...
  const rows = [
    ...
    ...data.idle.map(e=>({time:e.ended_at,type:"Idle interval",detail:`${duration(e.duration_seconds)}${e.start_is_censored ? " · Unknown beginning; excluded from advice" : ""}${e.end_is_censored ? " · Observation ended; no spin-up counted" : ""}`})),
...
async function refresh() {
  try {
    ...
    if (page === "dashboard") {
      ...
      $("disk-summary").innerHTML = metric("Current observed idle",`${d.current_idle_is_censored ? "≥ " : ""}${duration(d.current_idle_seconds)}`,d.current_idle_is_censored ? "Earlier activity is unknown" : "Since the last observed I/O") + metric("Longest / median",`${duration(d.longest_idle_seconds)} / ${duration(d.median_idle_seconds)}`,`${d.completed_interval_count} completed intervals`) + metric("75th / 90th percentile",`${duration(d.p75_idle_seconds)} / ${duration(d.p90_idle_seconds)}`,"Known complete intervals only") + metric("Valid observation",duration(d.valid_observation_seconds),`Last activity: ${timestamp(d.last_activity_at)}`);
